...
def render_vendas(can_edit_status=False):
    ...
    if st.session_state.carrinho:
        ...
        if cod_assoc:
            ...
            else: st.error(v["MSG_DESCONTO_ERRO"])
        ...
        if btn_c1.button(v["BOTAO_CONFIRMAR"], use_container_width=True, type="primary", key="venda_btn_confirmar"):
            try:
                ...
                session.rollback(); st.error(f"Erro ao salvar: {e}")
    ...
    d_ini = cf1.date_input("Início", datetime.date.today() - datetime.timedelta(days=30), key="venda_data_ini")
    d_fim = cf2.date_input("Fim", datetime.date.today(), key="venda_data_fim")
    ...
    if st.session_state.role == "admin":
        with st.expander(v["EXPANDER_ESTORNO"]):
            ...
            if st.button("Confirmar Estorno", type="secondary"):
